document.addEventListener('DOMContentLoaded', function () {
    ...
    function initDemo() {
        ...
        layoutFieldValue = layoutField.vadlue;
    ...
    function initGrid() {
        ...
        grid = new Muuri(gridElement, {
            items: [addCanvas('map-svg', 'Map Based Canvas'), addCanvas('force-svg', 'Force Directed Canvas')],
            layoutDuration: 400,
            layoutEasing: 'ease',
            dragEnabled: true,
            dragSortInterval: 50,
    ...
    function addCanvas(domId, title) {

        var id = ++uuid;
        var color = getRandomItem(filterOptions);
        var title = title// generateRandomWord(2);
        var width = Math.floor(Math.random() * 2) + 1;
        var height = Math.floor(Math.random() * 2) + 1;
        var itemElem = document.createElement('div');
        var itemTemplate = '' +
                '<div class="item" data-id="' + id + '" data-color="' + color + '" data-title="' + title + '">' +
